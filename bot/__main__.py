...
def start(update, context):
    ...
    buttons.buildbutton("Admin", "https://t.me/reconrex")
    buttons.buildbutton("Channel", "https://t.me/bdxleech")
    reply_markup = InlineKeyboardMarkup(buttons.build_menu(2))
    if CustomFilters.authorized_user(update) or CustomFilters.authorized_chat(update):
        start_string = f'''
Please Join @bdxleech to use this bot! 
'''
...
def restart(update, context):
    restart_message = sendMessage("♻️ Rebooting . . . ", context.bot, update)
    fs_utils.clean_all()
    alive.kill()
