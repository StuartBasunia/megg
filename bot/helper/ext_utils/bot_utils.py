...
class MirrorStatus:
    ...
    STATUS_CLONING = "♻️ Clonning . . ."
    STATUS_WAITING = "📝 Waiting . . . "
    STATUS_FAILED = "Failed to download!"
    STATUS_PAUSE = "Download Stopped!"
    STATUS_ARCHIVING = "🔐 Zipping . . ."
    STATUS_EXTRACTING = "📂 Unzipping . . ."
...
def get_readable_message():
    with download_dict_lock:
        ...
        for index, download in enumerate(list(download_dict.values())[START:], start=1):
            ...
            msg += f"\n<b>╔● Current Status: <i>{download.status()} </i></b>"
            if download.status() not in [
                MirrorStatus.STATUS_ARCHIVING,
                MirrorStatus.STATUS_EXTRACTING,
                MirrorStatus.STATUS_SPLITTING,
            ]:
                msg += f"\n<b>╟● Progress:</b> <code>{get_progress_bar_string(download)}</code>" \
                       f"\n<b>╟● Downloaded : {download.progress()} of 100% </b>"
                if download.status() == MirrorStatus.STATUS_CLONING:
                    msg += f"\n<b> Cloned :</b> <code>{get_readable_file_size(download.processed_bytes())}</code> of <code>{download.size()}</code>"
                elif download.status() == MirrorStatus.STATUS_UPLOADING:
                    msg += f"\n<b>╟● Upload : {get_readable_file_size(download.processed_bytes())} of {download.size()} </b>"
                else:
                    msg += f"\n<b>╟● Download : {get_readable_file_size(download.processed_bytes())} of {download.size()} </b>"
        ...
        bmsg = f"<b>🖥 CPU :</b> {psutil.cpu_percent()}% | <b>🔥 Free Space :</b> {free}"
        ...
        bmsg += f"\n<b>💾 RAM :</b> {psutil.virtual_memory().percent}% | <b>⏰ Uptime :</b> {currentTime}" \
                f"\n<b>╔●● Download :</b> {dlspeed}/s \n<b>╚●● Upload : </b> {ulspeed}/s"
        if STATUS_LIMIT is not None and dick_no > STATUS_LIMIT:
            msg += f"Downloading: {dick_no} files | Pages: {PAGE_NO}/{pages}\n"
