...
class AriaDownloadHelper:
            ...
            try:
                if STOP_DUPLICATE and dl is not None and not dl.getListener().isLeech:
                    ...
                    if not dl.getListener().extract:
                        ...
                        if smsg:
                            dl.getListener().onDownloadError('File already in the drive!\n\n')
                            api.remove([download], force=True)
                            sendMarkup("Click the button below:", dl.getListener().bot, dl.getListener().update, button)
                            return
    ...
    def __onDownloadStopped(self, api, gid):
        ...
        dl = getDownloadByGid(gid)
        if dl:
            dl.getListener().onDownloadError('🔴 Dead torrent vmro :/')
