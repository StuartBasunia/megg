...
def cancel_mirror(update, context):
    ...
    if len(args) == 1 and (
        ...
        msg = f"Please use the cancel command under your torrent in the status message!"
        sendMessage(msg, context.bot, update)
        return
    if dl.status() == MirrorStatus.STATUS_ARCHIVING:
        sendMessage("Zipping", context.bot, update)
    elif dl.status() == MirrorStatus.STATUS_EXTRACTING:
        sendMessage("Unzipping . . .", context.bot, update)
...
def cancel_all(update, context):
    ...
    while True:
        ...
        if dl:
            ...
            break
    sendMessage(f'{count} downloads has been cancelled!', context.bot, update)
