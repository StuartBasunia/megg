...
from bot import Interval, INDEX_URL, BUTTON_FOUR_NAME, BUTTON_FOUR_URL, BUTTON_FIVE_NAME, BUTTON_FIVE_URL, \
                ...
                dispatcher, DOWNLOAD_DIR, download_dict, download_dict_lock, ZIP_UNZIP_LIMIT, TG_SPLIT_SIZE, LOGGER, MEGA_KEY
...
class MirrorListener(listeners.MirrorListeners):
    ...
    def onUploadComplete(self, link: str, size, files, folders, typ):
        ...
        with download_dict_lock:
            ...
            link = short_url(link)
            buttons.buildbutton("☁️Google Dirve☁️", link)
...
def _mirror(bot, update, isZip=False, extract=False, isQbit=False, isLeech=False, pswd=None):
    ...
    if bot_utils.is_gdrive_link(link):
        if not isZip and not extract and not isLeech:
            sendMessage(f"To copy from google drive use /{BotCommands.CloneCommand} \nIf you want to make a zip file use /{BotCommands.ZipMirrorCommand}\nIf you want to unzip a file use /{BotCommands.UnzipMirrorCommand}", bot, update)
            return
        ...
        if BLOCK_MEGA_LINKS:
            sendMessage("Sorry, I cant download from mega.nz right now . . .", bot, update)
            return
